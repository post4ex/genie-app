...
export default function VaultScreen({ token = '', apiBase = '', user = {} }) {
  ...
  const renderRecord = (row, index) => {
    ...
    const date = dateValue(row.DOX_DATE || row.IO_TIMESTAMP || row.TXN_DATE || row.ENTRY_DATE);
    const subtitle = [
      row.CODE || row.BRANCH || row.COMPANY_NAME || row.DOX_TYPE || row.ENTRY_TYPE || row.description || row.state || '',
      date !== '—' ? date : '',
    ].filter(Boolean);
    const status = row.STATUS || row.state || undefined;
    const isSelected = selected && keyOf(activeTile, selected) === id;
    return (
      <ListItem
        key={`${id}-${index}`}
        title={`${row.icon || ''} ${String(name)}`.trim()}
        subtitle={subtitle}
        status={status}
        trailing={amount != null ? (
          <View style={styles.recordTrailing}>
            <Text style={styles.recordAmount}>{typeof amount === 'number' || !Number.isNaN(Number(amount)) ? money(amount) : String(amount || '—')}</Text>
          </View>
        ) : null}
        onPress={() => setSelected(row)}
        style={isSelected ? styles.recordSelectedListItem : undefined}
      />
    );
  };
