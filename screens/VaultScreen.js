...
export default function VaultScreen({ token = '', apiBase = '', user = {} }) {
  ...
  useEffect(() => { reload(); if (branchRequired && !selectedBranch) setBranchModal(true); }, []);
  ...
  const openTile = async (tile) => {
    ...
    if (branchRequired && !branch) { setBranchModal(true); return; }
