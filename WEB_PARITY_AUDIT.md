...
| `uploader.html` | Full uploader: order pane/search/date filter/load-more, pickup-task table, camera state machine, multiple images, PDF pages, crop, enhancement, OCR, barcode, rotation/lock/cancel, row-specific fields, retry/delete/clear/submit. | **Partial/Native adaptation** — `UploaderScreen` has the major port and native modules. | Live visual and interaction comparison remains required; verify all task types, PDF viewer/preview, crop gestures, enhancement bytes, OCR filling, submit/retry and row isolation. Native modules already installed: OCR/PDF page image/view-shot/camera. |
...
| `jawaS/uploader.js` | Full uploader order list, task rows, state transitions, submit/retry. | **Partial/native adaptation** — `UploaderScreen`. |
| `jawaS/mini-uploader.js` | Shipment/booking mini-uploader task restrictions, row inputs, uploads, previews and submit. | **Partial/native adaptation** — reused `UploaderScreen` modal; needs exact mini layout/flow verification. |
